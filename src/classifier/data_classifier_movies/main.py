...
async def main():
    ...
    for the_filter in ["known", "unknown"]:

        feature_activations_known_1Q, feature_library_known_1Q = await get_feature_activations_v2(
            client_gf,
            variant,
            queries_for_feature_extraction,
            known_unknown_filter=the_filter,
            key_name="query",
            k=100,
        )

        # Pickle results
        with open(f"feature_activations_{the_filter}_1Q.pkl", "wb") as f:
            pickle.dump(feature_activations_known_1Q, f)

        with open(f"feature_library_{the_filter}_1Q.pkl", "wb") as f:
